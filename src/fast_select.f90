! Author: Thomas C.H. Lux (Virginia Polytechnic)
!
MODULE FAST_SELECT
...
CONTAINS
  ...
  !   VALUES(K) has rank K.
  ! 
  RECURSIVE SUBROUTINE Quick_Select(VALUES, K, DIVISOR, MAX_SIZE)
    ! Arguments
    ...
    DO WHILE (LEFT .LT. RIGHT)
       ...
       IF (RIGHT - LEFT .GT. MS) THEN
          ...
          CALL Quick_Select(VALUES(L:R), K - L + 1, DIVISOR, MAX_SIZE)
       END IF
  ...
  RECURSIVE SUBROUTINE FLOYD_RIVEST(ARRAY, K, L, R)
    ! Arguments
    REAL(KIND=REAL32), INTENT(INOUT), DIMENSION(:) :: ARRAY
    INTEGER, INTENT(IN) :: K
    INTEGER, INTENT(IN), OPTIONAL :: L, R
    ! Locals
    INTEGER :: I, J, NEWLEFT, NEWRIGHT, LEFT, RIGHT
    REAL(KIND=REAL32) :: N, Z, S, SD, T
    ! Handle "L" optional.
    ...
    DO WHILE (RIGHT .GT. LEFT)
       ...
       IF (RIGHT - LEFT .GT. 600) THEN
          ...
          IF (I .LT. N/2) SD = -SD
          NEWLEFT = MAX(REAL(LEFT,REAL32), K - I * S/N + SD)
          NEWRIGHT = MIN(REAL(RIGHT,REAL32), K + (N - I) * S/N + SD)
  ...
  SUBROUTINE SWAP(V1, V2)
    REAL(KIND=REAL32), INTENT(INOUT) :: V1, V2
    ! Local temp
  ...
  SUBROUTINE BUBBLE_SORT(VALUES)
    REAL(KIND=REAL32), DIMENSION(:), INTENT(INOUT) :: VALUES
    ! Local variables
    INTEGER :: I, J, K
  ...
  FUNCTION PARTITION(VALUES, PIVOT, INDEX) RESULT(LOC)
    ...
    INTEGER :: LOC
    ! Local variables
    REAL(KIND=REAL32) :: TEMP, PIVOT_VALUE
  ...
  SUBROUTINE INTROSELECT(VALUES, INDEX, ALLOWANCE)
    ...
    REAL(KIND=REAL32), INTENT(IN), OPTIONAL :: ALLOWANCE
    ! Local variables
    INTEGER :: PIVOT, I, SEARCHED, FIRST, LAST
    REAL(KIND=REAL32) :: PIVOT_VALUE, LOCAL_ALLOWANCE
    ...
    ELSE                         ; LOCAL_ALLOWANCE = 5.0_REAL32
    END IF
